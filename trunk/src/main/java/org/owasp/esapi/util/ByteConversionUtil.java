...
public class ByteConversionUtil {
  ...
  /**
   * Returns a byte array containing 8 network byte-ordered bytes representing the
   * given {@code long}.
   *
   * @param input The {@code long} to convert to a byte array.
  ...
  public static long toLong(byte[] input) {
    ...
    long output = 0;
    output = ((input[0] & 0xff) << 56) | ((input[1] & 0xff) << 48) |
