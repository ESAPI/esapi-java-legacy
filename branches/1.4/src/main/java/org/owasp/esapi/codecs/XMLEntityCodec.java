...
public class XMLEntityCodec extends AbstractCodec
{
	...
	{	// populate entitites
		...
		entityToCharacterMap.put("lt", PrimWrap.wrapChar('<'));
		entityToCharacterMap.put("gt", PrimWrap.wrapChar('>'));
		entityToCharacterMap.put("amp", PrimWrap.wrapChar('&'));
		entityToCharacterMap.put("apos", PrimWrap.wrapChar('\''));
		entityToCharacterMap.put("quot", PrimWrap.wrapChar('"'));
	}
	...
	public Character decodeCharacter(PushbackString input)
	{
		...
		try
		{
			...
			secondCh = second.charValue();
			if(secondCh =='#')
			{	// handle numbers
				ret = getNumericEntity(input);
			}
			else if(Character.isLetter(secondCh))
			{	// handle entities
	...
	{
		/* functionality doesn't exist in 1.4...
